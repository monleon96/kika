...
This document describes the consistent debug logging system implemented for all ENDF parsers in KIKA.
...
modules = ['kika.endf.parsers.parse_mf4']
configure_endf_debug_logging(enable=True, modules=modules)

# Debug MF4 and MF34 parsers
modules = ['kika.endf.parsers.parse_mf4', 'kika.endf.parsers.parse_mf34']
configure_endf_debug_logging(enable=True, modules=modules)

# Debug all ENDF parsers
modules = [
    'kika.endf.parsers.parse_endf',
    'kika.endf.parsers.parse_mf1', 
...
Examples:
```
[DEBUG] kika.endf.parsers.parse_endf: Starting to parse ENDF file: example.endf
[DEBUG] kika.endf.parsers.parse_mf1: Parsing MF1 with 45 lines
[DEBUG] kika.endf.parsers.parse_mf34: LB=5 processing: NE=20
[WARNING] kika.endf.parsers.parse_mf4: Error parsing MT2 in MF4: Invalid data
```
...
- `kika.endf.parsers.parse_endf`: Main ENDF file parser
- `kika.endf.parsers.parse_mf1`: MF1 (General Information) parser
- `kika.endf.parsers.parse_mf4`: MF4 (Angular Distributions) parser
- `kika.endf.parsers.parse_mf34`: MF34 (Angular Distribution Covariances) parser
...
6. **Use the example notebook**: See `kika/endf/classes/mf4/test_plots.ipynb` for practical usage patterns
...
configure_endf_debug_logging(enable=True, modules=['kika.endf.parsers.parse_mf4'])
# Parse MF4 only  
...
**New way (consistent):**
```python
from kika.utils import get_endf_logger
...
- All parsers use the same logger hierarchy under `kika.endf.parsers.*`
...
For more examples, see `kika/endf/classes/mf4/test_plots.ipynb`.
