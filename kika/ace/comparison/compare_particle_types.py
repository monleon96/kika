...
def compare_particle_types(ace1: Ace, ace2: Ace, tolerance: float = 1e-6, verbose: bool = True) -> bool:
    ...
    has_particle_types1 = (ace1.secondary_particle_types is not None and ace1.secondary_particle_types.has_data)
    has_particle_types2 = (ace2.secondary_particle_types is not None and ace2.secondary_particle_types.has_data)
    ...
    if len(ace1.secondary_particle_types.particle_ids) != len(ace2.secondary_particle_types.particle_ids):
        if verbose:
            print(f"Particle production types mismatch: Number of particle types differs "
                  f"({len(ace1.secondary_particle_types.particle_ids)} vs {len(ace2.secondary_particle_types.particle_ids)})")
        return False
    
    # Compare particle IDs
    particle_ids1 = ace1.secondary_particle_types.particle_ids
    particle_ids2 = ace2.secondary_particle_types.particle_ids
