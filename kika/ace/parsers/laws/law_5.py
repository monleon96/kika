...
from kika.ace.classes.energy_distribution.distributions.evaporation import GeneralEvaporationSpectrum

# Setup logger
logger = logging.getLogger(__name__)

def parse_general_evaporation_spectrum(ace: Ace, base_dist: EnergyDistribution, idat_idx: int, debug: bool = False) -> GeneralEvaporationSpectrum:
    """
    ...
    """
    if debug:
        logger.debug(f"Parsing general evaporation spectrum (Law 5) starting at index {idat_idx}")
    ...
    if debug:
        logger.debug(f"Completed parsing general evaporation spectrum with {n_e} incident energies and {net} equiprobable bins")
    return distribution