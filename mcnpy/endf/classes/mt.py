...
from ..utils import format_endf_data_line
...
class MT451:
    ...
    def directory(self) -> pd.DataFrame:
        ...
        Returns:
            DataFrame with columns: MF, MT, rows
            - MF: File number
            - MT: Section number 
            - rows: Number of records in the section
        """
        if not self._directory:
            # Return empty DataFrame with correct columns
            return pd.DataFrame(columns=['MF', 'MT', 'rows'])
            
        # Convert the list of tuples to DataFrame with selected columns
        df = pd.DataFrame(self._directory, columns=['MF', 'MT', 'NC', 'MOD'])
        
        # Rename NC to rows and drop MOD column
        df = df.rename(columns={'NC': 'rows'}).drop(columns=['MOD'])
