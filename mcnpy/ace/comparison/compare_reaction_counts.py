...
def compare_reaction_counts(ace1: Ace, ace2: Ace, tolerance: float = 1e-6, verbose: bool = True) -> bool:
    ...
    has_counts1 = (ace1.particle_reaction_counts is not None and ace1.particle_reaction_counts.has_data)
    has_counts2 = (ace2.particle_reaction_counts is not None and ace2.particle_reaction_counts.has_data)
    ...
    if len(ace1.particle_reaction_counts.reaction_counts) != len(ace2.particle_reaction_counts.reaction_counts):
        if verbose:
            print(f"Particle reaction counts mismatch: Number of entries differs "
                  f"({len(ace1.particle_reaction_counts.reaction_counts)} vs "
                  f"{len(ace2.particle_reaction_counts.reaction_counts)})")
        return False
    
    # Compare reaction counts for each particle type
    counts1 = ace1.particle_reaction_counts.reaction_counts
    counts2 = ace2.particle_reaction_counts.reaction_counts
