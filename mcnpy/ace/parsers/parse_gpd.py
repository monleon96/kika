...
NEUTRON_ENERGY_BOUNDARIES = [
    ...
    1.235e-3, 3.35e-3, 9.2e-3, 2.48e-2, 6.76e-2,
...
def read_gpd_block(ace, debug=False):
    ...
    # Get the number of energy points from the ESZ block
    ...
    ace.photon_production_data.total_xs = [ace.xss_data[gpd_idx + i] for i in range(n_energy)]
    ...
    if debug:
        ...
        logger.debug(f"Need {outgoing_size} additional values for outgoing energies")
    
    # There should be 30 groups with 20 energies each = 600 values
    if outgoing_start + outgoing_size <= len(ace.xss_data):
        if debug:
            logger.debug("Outgoing photon energies data found")
        ...
        ace.photon_production_data.outgoing_energies = outgoing_energies
        
        # Also store the neutron energy boundaries for reference
        ace.photon_production_data.neutron_energy_boundaries = NEUTRON_ENERGY_BOUNDARIES
